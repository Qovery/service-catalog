locals {
  ...
  # A sharded group needs the cluster-mode parameter group family; default.valkey8 is rejected there.
  parameter_group_name = (
    var.parameter_group_name != "" ? var.parameter_group_name
    : local.cluster_mode ? "default.valkey8.cluster.on" : "default.valkey8"
  )
}
