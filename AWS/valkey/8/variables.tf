...
variable "engine_version" {
  type        = string
  default     = "8.0"
  description = "Valkey engine version (8.0, 8.1 or 8.2). All three share the valkey8 parameter group family."

  validation {
    condition     = contains(["8.0", "8.1", "8.2"], var.engine_version)
    error_message = "engine_version must be 8.0, 8.1 or 8.2 (this blueprint is the Valkey 8 major)."
  }
}
...
variable "parameter_group_name" {
  type        = string
  default     = ""
  description = "Leave empty — derived from the topology: default.valkey8 for a single node group, default.valkey8.cluster.on when instances_number > 1."
}
