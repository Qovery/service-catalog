# AWS ElastiCache for Valkey 9

Creates an AWS ElastiCache for Valkey 9 replication group with configurable node type, topology, backups, maintenance window, and network settings. Traffic is encrypted in transit and protected by an auth token, and data is encrypted at rest.
...
The `rediss://` scheme is correct here and not a leftover: Valkey speaks the Redis protocol, so Redis clients and URLs work unchanged — which is what makes it a drop-in replacement. Engine versions `9.0` and `9.1` share the `valkey9` parameter group family, so either can be selected without changing the derived parameter group. AWS states that Valkey 9 parameter groups are incompatible with Valkey 8.x and Redis, which is why Valkey 9 is its own blueprint rather than a version choice inside the Valkey 8 one.
...
| ---- | ---- | ------- | ----------- |
| `engine_version` | string | `9.0` | Valkey engine version: `9.0`, `9.1` |
...
`instances_number = 1` creates a single node with no replica. Above 1, the group switches to cluster mode: `N` shards, one replica each (so `2N` nodes), with automatic failover and Multi-AZ enabled because AWS requires both there, and the `default.valkey9.cluster.on` parameter group instead of `default.valkey9`. Clients then need a cluster-aware Valkey library and the `valkey_configuration_endpoint_address` output.
