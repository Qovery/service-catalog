locals {
  ...
  redis_identifier = var.import_identifier != "" ? var.import_identifier : lower(var.redis_name)
  ...
  # A sharded group needs the cluster-mode parameter group family; default.redis7 is rejected there.
  parameter_group_name = (
    var.parameter_group_name != "" ? var.parameter_group_name
    : local.cluster_mode ? "default.redis7.cluster.on" : "default.redis7"
  )
}
...
locals {
  redis_password = var.redis_password != "" ? var.redis_password : random_password.auth_token.result
}
...
resource "aws_elasticache_replication_group" "this" {
  ...
  # would leave a Redis reachable with no password at all.
  ...
  tags = {
    ...
    Blueprint     = "aws-elasticache-redis"
